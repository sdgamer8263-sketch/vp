...
export const Users = () => {
  ...
  const users = [
    { id: 1, email: "admin@example.com", username: "admin", permissions: "All Permissions", has2FA: true },
    { id: 2, email: "moderator@example.com", username: "moderator", permissions: "Console, Start, Stop", has2FA: false },
  ];
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Manage sub-users and their permissions.</p>
        </div>
        <button 
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/10 bg-white/5">
                ...
                <th className="px-6 py-4 font-semibold text-gray-300 text-sm">Permissions</th>
                <th className="px-6 py-4 font-semibold text-gray-300 text-sm">2FA Status</th>
              ...
              {users.map((user) => (
                <tr key={user.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-6 py-4 text-gray-400">
                    <span className="bg-black/40 px-2 py-1 rounded text-xs border border-white/5">{user.permissions}</span>
                  </td>
                  <td className="px-6 py-4">
                    {user.has2FA ? (
                      <span className="flex items-center gap-1 text-green-400 text-xs font-medium px-2 py-1 bg-green-500/10 rounded-full border border-green-500/20 w-fit">
                        <Shield size={12} /> Enabled
                      </span>
                    ) : (
                      <span className="flex items-center gap-1 text-red-400 text-xs font-medium px-2 py-1 bg-red-500/10 rounded-full border border-red-500/20 w-fit">
                        <Shield size={12} /> Disabled
                      </span>
                    )}
                  </td>
